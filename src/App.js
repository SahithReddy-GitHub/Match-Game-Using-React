...
class App extends Component {
  constructor(props) {
    ...
    this.state = {
      score: 0,
      seconds: 60,
      imgData: imagesList[0],
      idTab: tabsList[0].tabId,
      isGameOver: false,
    }
  ...
  onImageClick = id => {
    ...
    if (imgData.id === id) {
      this.setState(prevState => ({
        ...
        imgData: imagesList[Math.floor(Math.random() * imagesList.length)],
      }))
    } else {
      this.setState(prevState => ({isGameOver: !prevState.isGameOver}))
      clearInterval(this.interval)
  ...
  render() {
    const {score, seconds, imgData, idTab, isGameOver} = this.state
    const {imageUrl} = imgData
    const filteredImageList = imagesList.filter(
      eachImageData => eachImageData.category === idTab,
    )
          ...
          <li className="head-con">
            ...
            <div className="time-con">
              <img
                ...
                className="timer-img"
              />
              <p className="para-ext">{seconds} sec</p>
            </div>
          </li>
        </ul>
        <div className="bg">
          {!isGameOver && (
            <div className="con">
              ...
              <div>
                <ul className="ul-extra">
                  {tabsList.map(eachTab => (
                    <TabItem
                      isActive={idTab === eachTab.tabId}
                      tabData={eachTab}
                      key={eachTab.tabId}
              ...
              <ul className="ul-extra image-custom">
            ...
            <div className="con2">
              ...
              >
                <img
                  ...
                  className="timer-img"
                />
                <p className="again-head">PLAY AGAIN</p>
              </button>
            </div>
          )}
        </div>
